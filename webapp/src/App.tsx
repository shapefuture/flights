...
// Create a component for displaying errors
...
function App() {
  ...
  useEffect(() => {
    const checkExtension = async () => {
      try {
        ...
        logger.debug('Extension status:', status);
        setExtensionStatus(status);
      } catch (error) {
        logger.error('Error checking extension status:', error);
        setExtensionStatus({ installed: false, active: false });
      }
    };
    ...
    try {
      ...
      if (savedSearchesFromStorage) {
        const parsedSearches = JSON.parse(savedSearchesFromStorage);
        logger.debug('Loaded saved searches:', parsedSearches);
        setSavedSearches(parsedSearches);
      }
    } catch (e) {
      logger.error('Error loading saved searches:', e);
    }
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      setError(`Error filtering results: ${error instanceof Error ? error.message : String(error)}`);
    }
  }, [filters, results]);
  ...
  useEffect(() => {
    ...
    try {
      cleanupFunction = listenForExtensionMessages((message) => {
        ...
        try {
          ...
          setError(`Error processing data from extension: ${error instanceof Error ? error.message : String(error)}`);
        }
      });
    } catch (error) {
      logger.error('Error setting up extension message listener:', error);
      setError(`Error communicating with extension: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    return () => {
      try {
        ...
        logger.error('Error cleaning up extension message listener:', error);
      }
    };
  ...
  const sendNextBatch = useCallback(async () => {
    if (pendingQueries.length === 0 || cancelRequested) {
      logger.debug('No pending queries or cancel requested, skipping batch');
      return;
    }
    ...
    logger.debug(`Sending batch of ${batch.length} queries, ${remaining.length} remaining`);
    ...
    try {
      ...
      logger.error('Failed to communicate with extension:', e);
      setError(`Failed to communicate with extension: ${e instanceof Error ? e.message : String(e)}`);
    }
  }, [pendingQueries, cancelRequested, batchSize]);
  ...
  useEffect(() => {
    ...
    const executeCurrentStep = async () => {
      logger.debug(`Executing step ${currentStepIndex + 1} of ${plan.steps.length}`);
      ...
      try {
        if (step.action === 'generate_search_queries') {
          logger.debug('Generating search queries with parameters:', step.parameters);
          ...
          logger.debug('Executing flight fetch');
          
          // If extension is not available, show error and skip
          if (!extensionStatus.active) {
            logger.warn('Browser extension is not active, skipping flight fetch');
          ...
          setResults(prev => {
            ...
            try {
              ...
              return resultsToSort; // Return unsorted on error
            }
          });
    ...
    executeCurrentStep();
  }, [plan, currentStepIndex, extensionStatus.active, cancelRequested, sendNextBatch]);
  ...
  const filterResults = useCallback(() => {
    ...
    try {
      logger.debug('Filtering results with criteria:', filters);
      ...
      logger.debug(`Filtered from ${results.length} to ${filtered.length} results`);
      setFilteredResults(filtered);
    } catch (error) {
      logger.error('Error in filterResults:', error);
      // Don't update filtered results on error
    }
  }, [results, filters]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    logger.info('Submitting search query:', query);
    setIsLoading(true);
    setThinking('');
    setPlan(null);
    setResults([]);
  ...
  const handleFeedback = async (feedback: string) => {
    ...
    try {
      ...
      if (response.error) {
        logger.error('API returned error for feedback:', response.error);
        setError(response.error);
      } else {
        logger.debug('API response for feedback:', response);
        setThinking(response.thinking || '');
        setPlan(response.plan);
  ...
  const saveSearch = useCallback(() => {
    ...
    try {
      logger.debug('Saving search:', query);
      const updatedSearches = [...savedSearches];
      if (!updatedSearches.includes(query)) {
        updatedSearches.push(query);
        setSavedSearches(updatedSearches);
        localStorage.setItem('savedSearches', JSON.stringify(updatedSearches));
        logger.debug('Search saved successfully');
      } else {
        logger.debug('Search already saved');
      }
    } catch (error) {
      logger.error('Error saving search:', error);
      setError(`Failed to save search: ${error instanceof Error ? error.message : String(error)}`);
    }
  }, [query, savedSearches]);
  
  const loadSavedSearch = useCallback((savedQuery: string) => {
    try {
      logger.debug('Loading saved search:', savedQuery);
      setQuery(savedQuery);
    } catch (error) {
      logger.error('Error loading saved search:', error);
      setError(`Failed to load saved search: ${error instanceof Error ? error.message : String(error)}`);
    }
  }, []);
  ...
  const calculateStats = useCallback(() => {
    ...
    try {
      logger.debug('Calculating stats for filtered results');
  ...
  const handleResetFilters = useCallback(() => {
    try {
      ...
      setError(`Failed to reset filters: ${error instanceof Error ? error.message : String(error)}`);
    }
  }, []);
  ...
  // Calculate stats only when filteredResults changes
                              ...
                              <SavedSearches 
                                ...
                                onDelete={(search) => {
                                  try {
                                    ...
                                    setError(`Failed to delete saved search: ${error instanceof Error ? error.message : String(error)}`);
                                  }
                                }}
                              />
                            </TabsContent>
